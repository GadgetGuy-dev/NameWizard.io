...
export async function generateNameSuggestions(data: { 
  ...
  try {
    ...
    const userContent = `I need you to suggest better names for this file: "${data.fileName}".
    ...
    if (message.content.length > 0 && 'text' in message.content[0]) {
      ...
      console.log('Response content format unexpected:', message.content);
      responseText = '{}';
    }
...
export async function analyzeContent(data: {
  ...
  try {
    ...
    const userContent = `I need you to analyze these files and suggest logical categories for organizing them:
    ${fileInfoText}
...
export async function formatCase(names: string[], caseType: string) {
  ...
  try {
    const userContent = `I need you to format these file names to ${caseType}:
    ${names.join('\n')}
...
export async function extractMetadata(fileName: string, content: string) {
  ...
  try {
    const userContent = `I need you to extract relevant metadata from this file:
    Filename: ${fileName}
...
export async function organizeFoldersWithClaude(files: Array<{
  ...
  try {
    ...
    const userContent = `I need you to organize these files into a logical folder structure:
    ${fileInfoText}
...
export async function detectDuplicates(files: Array<{
  ...
  try {
    ...
    const userContent = `I need you to analyze these files and identify potential duplicates or naming conflicts:
    ${fileInfoText}
