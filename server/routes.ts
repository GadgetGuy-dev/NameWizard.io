...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Real API key validation - actually calls the provider APIs
  app.post("/api/validate-api-key", async (req, res) => {
    try {
      ...
      console.log(`Validating API key for provider: ${type}`);
      ...
      switch (type) {
        case 'openai': {
          // Check for OpenRouter key format entered in OpenAI field
          if (key.startsWith('sk-or-')) {
            return res.json({
              success: false,
              message: "This appears to be an OpenRouter key (starts with 'sk-or-'). Please enter it in the OpenRouter provider section, not OpenAI."
            });
          }
          ...
          try {
            ...
            success = true;
            message = `OpenAI key valid. Found ${response.data.length} models available.`;
          } catch (error: any) {
            latency = Date.now() - startTime;
            success = false;
            message = error?.message || "Failed to validate OpenAI key";
            // Parse OpenAI error messages for cleaner display
            if (error?.status === 401) {
              message = "Invalid API key. Please check your key at https://platform.openai.com/api-keys";
            } else if (error?.status === 429) {
              message = "Rate limited. The key is valid but has hit usage limits.";
        ...
        case 'anthropic': {
          try {
            ...
            latency = Date.now() - startTime;
            success = true;
            message = "Anthropic key validated successfully.";
          } catch (error: any) {
            latency = Date.now() - startTime;
            success = false;
            message = error?.message || "Failed to validate Anthropic key";
            // Parse Anthropic error messages for cleaner display
            if (error?.status === 401) {
              message = "Invalid API key. Please check your key at https://console.anthropic.com/settings/keys";
            } else if (error?.status === 429) {
              message = "Rate limited. The key is valid but has hit usage limits.";
              success = true; // Key is valid, just rate limited
            }
        ...
        case 'google': {
          ...
          try {
            ...
            if (response.ok) {
              ...
              success = true;
              message = `Google API key valid. Found ${data.models?.length || 0} models available.`;
            } else {
              const error = await response.json();
              success = false;
              message = error?.error?.message || "Invalid Google API key";
            }
          } catch (error: any) {
            latency = Date.now() - startTime;
            success = false;
        ...
        case 'openrouter': {
          try {
            ...
            if (response.ok) {
              const data = await response.json();
              success = true;
              message = `OpenRouter key valid. Access to ${data.data?.length || 0} models.`;
            } else {
              success = false;
            ...
            latency = Date.now() - startTime;
            success = false;
            message = error?.message || "Failed to validate OpenRouter key";
          }
          break;
        }
        ...
        case 'mistral': {
          try {
            ...
            latency = Date.now() - startTime;
            success = false;
            message = error?.message || "Failed to validate Mistral key";
          }
          break;
        }
          ...
          try {
            ...
            if (response.ok || response.status === 429) {
              ...
              message = response.status === 429 
                ? "Perplexity key valid (rate limited)." 
            ...
            latency = Date.now() - startTime;
            success = false;
            message = error?.message || "Failed to validate Perplexity key";
          }
          break;
        }
        ...
        case 'xai': {
          ...
          try {
            ...
            if (response.ok) {
              success = true;
              message = "xAI Grok API key validated successfully.";
            } else {
              success = false;
            ...
            latency = Date.now() - startTime;
            success = false;
            message = error?.message || "Failed to validate xAI key";
          }
          break;
        }
        ...
        case 'googledrive': {
          ...
          if (key.length > 20) {
            success = true;
            message = "Cloud storage key format validated. Full validation requires OAuth flow.";
          } else {
            success = false;
            message = "API key appears too short. Please check the key format.";
          }
          break;
        }
        ...
        case 'google-vision': {
          try {
            ...
            if (data.error && data.error.code === 400) {
              success = true;
              message = "Google Cloud Vision API key validated successfully.";
            } else if (data.error && (data.error.code === 401 || data.error.code === 403)) {
              success = false;
              message = "Invalid Google Cloud Vision API key.";
            } else {
              success = true;
              message = "Google Cloud Vision API key validated.";
            }
          } catch (error: any) {
            latency = Date.now() - startTime;
            success = false;
            message = error?.message || "Failed to validate Google Vision key";
          }
          break;
        }
        ...
        case 'azure-vision': {
          ...
          if (key.length > 20) {
            success = true;
            message = `OCR provider key format validated. Configure the service endpoint for full integration.`;
          } else {
            success = false;
            message = "API key appears too short. Please check the key format.";
          }
        ...
        default:
          success = false;
          message = `Validation not implemented for provider: ${type}`;
      }
  ...
  // Check for model updates from provider APIs
  app.post("/api/check-model-updates", async (req, res) => {
    try {
      const { type, key } = req.body;
      ...
      console.log(`Checking model updates for provider: ${type}`);
      ...
      switch (type) {
        case 'openai': {
          try {
            const openai = new OpenAI({ apiKey: key });
            const response = await openai.models.list();
            models = response.data.map(m => m.id).filter(id => 
              id.includes('gpt') || id.includes('o1') || id.includes('o3')
            );
            message = `Found ${models.length} GPT models available.`;
          } catch (error: any) {
            message = "Could not fetch models. Check API key.";
          }
          break;
        }
        ...
        case 'anthropic': {
          ...
          message = "Anthropic models are pre-configured. No new models detected.";
          break;
        }
        ...
        case 'google': {
          try {
            ...
            if (response.ok) {
              ...
              message = `Found ${models.length} Gemini models available.`;
            }
          } catch (error) {
            message = "Could not fetch Google models.";
          }
          break;
        }
        ...
        case 'openrouter': {
          try {
            ...
            if (response.ok) {
              const data = await response.json();
              models = data.data?.slice(0, 20).map((m: any) => m.id) || [];
              message = `OpenRouter provides access to ${data.data?.length || 0} models.`;
            }
          } catch (error) {
            message = "Could not fetch OpenRouter models.";
          }
          break;
        }
        
        default:
          message = `Model listing not available for ${type}. Configuration is up to date.`;
      }
      ...
      return res.status(500).json({
        success: false,
        message: error?.message || "Error checking model updates"
      });
    }
  });
  ...
  app.get("/api/admin/providers-health", async (req, res) => {
    try {
      const metrics = await storage.getAllApiMetrics();
      const providers = metrics.map(m => ({
        id: m.providerName,
        name: m.providerName,
        status: m.errorCount > m.successCount * 0.1 ? 'error' : 'active',
        latency: m.requestCount > 0 ? Math.round((m.totalLatencyMs || 0) / m.requestCount) : 0,
        lastChecked: m.createdAt,
        errorMessage: m.errorCount > 0 ? `${m.errorCount} errors in last session` : null,
        category: ['google-vision', 'aws-textract', 'azure-vision', 'techvision'].includes(m.providerName.toLowerCase()) 
          ? 'ocr' 
          : ['dropbox', 'googledrive'].includes(m.providerName.toLowerCase()) 
            ? 'cloud' 
            : 'ai'
      }));
      return res.json(providers);
    } catch (error: any) {
      console.error("Error fetching providers health:", error);
      return res.status(500).json({ success: false, message: error?.message || "Failed to fetch providers health" });
    }
  });
