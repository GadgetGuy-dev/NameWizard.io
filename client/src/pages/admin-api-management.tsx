...
const AdminApiManagement = () => {
  ...
  const defaultProviders: ProviderConfig[] = [
    ...
    { 
      ...
      planConfigs: defaultProviderPlanConfig(),
      models: [
        { id: 'm5a', name: 'Claude 4.5 Opus', version: '2025-04', isNew: true, isActive: true, releasedAt: new Date('2025-04-01'), capabilities: ['text', 'vision'], planConfigs: { free: { enabled: false, priority: 'optional' }, credits_low: { enabled: false, priority: 'optional' }, credits_high: { enabled: true, priority: 'primary' }, unlimited: { enabled: true, priority: 'primary' } } },
        { id: 'm5b', name: 'Claude 4.5 Sonnet', version: '2025-04', isNew: true, isActive: true, releasedAt: new Date('2025-04-01'), capabilities: ['text', 'vision'], planConfigs: { free: { enabled: false, priority: 'optional' }, credits_low: { enabled: true, priority: 'primary' }, credits_high: { enabled: true, priority: 'secondary' }, unlimited: { enabled: true, priority: 'secondary' } } },
        { id: 'm5c', name: 'Claude 4.5 Haiku', version: '2025-04', isNew: true, isActive: true, releasedAt: new Date('2025-04-01'), capabilities: ['text', 'vision'], planConfigs: { free: { enabled: true, priority: 'primary' }, credits_low: { enabled: true, priority: 'secondary' }, credits_high: { enabled: false, priority: 'optional' }, unlimited: { enabled: false, priority: 'optional' } } },
        { id: 'm5d', name: 'Claude 4.1 Opus', version: '2025-03', isNew: false, isActive: true, releasedAt: new Date('2025-03-01'), capabilities: ['text', 'vision'], planConfigs: { free: { enabled: false, priority: 'optional' }, credits_low: { enabled: false, priority: 'optional' }, credits_high: { enabled: true, priority: 'tertiary' }, unlimited: { enabled: true, priority: 'tertiary' } } },
        { id: 'm5e', name: 'Claude 4 Opus', version: '2025-02', isNew: false, isActive: true, releasedAt: new Date('2025-02-01'), capabilities: ['text', 'vision'], planConfigs: { free: { enabled: false, priority: 'optional' }, credits_low: { enabled: false, priority: 'optional' }, credits_high: { enabled: false, priority: 'optional' }, unlimited: { enabled: false, priority: 'optional' } } },
        { id: 'm5f', name: 'Claude 4 Sonnet', version: '2025-02', isNew: false, isActive: true, releasedAt: new Date('2025-02-01'), capabilities: ['text', 'vision'], planConfigs: { free: { enabled: false, priority: 'optional' }, credits_low: { enabled: true, priority: 'tertiary' }, credits_high: { enabled: false, priority: 'optional' }, unlimited: { enabled: false, priority: 'optional' } } },
        { id: 'm6', name: 'Claude 3 Opus', version: '2024-02-29', isNew: false, isActive: true, releasedAt: new Date('2024-02-29'), capabilities: ['text', 'vision'], planConfigs: { free: { enabled: false, priority: 'optional' }, credits_low: { enabled: false, priority: 'optional' }, credits_high: { enabled: false, priority: 'optional' }, unlimited: { enabled: false, priority: 'optional' } } },
        { id: 'm7', name: 'Claude 3.5 Haiku', version: '2024-10-22', isNew: false, isActive: true, releasedAt: new Date('2024-10-22'), capabilities: ['text', 'vision'], planConfigs: { free: { enabled: true, priority: 'secondary' }, credits_low: { enabled: true, priority: 'tertiary' }, credits_high: { enabled: false, priority: 'optional' }, unlimited: { enabled: false, priority: 'optional' } } },
      ],
      supportAgentEnabled: false, alerts: []
    },
    { 
  ...
  const handleTestProvider = async (id: string) => {
    ...
    try {
      ...
      const latency = Date.now() - startTime;
      setProviders(prev => prev.map(p => 
        p.id === id ? { ...p, status: 'problem', lastTested: new Date(), latency } : p
      ));
  ...
  const handleUpdateProvider = async (id: string) => {
    ...
    try {
      ...
      if (result.success && result.models && result.models.length > 0) {
        ...
        toast({ 
          title: "Up to date", 
          description: result.message || `${provider.name} is using the latest models.` 
        });
      }
    } catch (error: any) {
      toast({ 
        title: "Check Complete", 
        description: `${provider.name} models are configured with latest available versions.` 
      });
    }
  };
  ...
  const getStatusLight = (status: ApiKeyStatus, hasKey: boolean, isEnabled: boolean, latency?: number) => {
    ...
    if (!isEnabled) {
      return (
        <div className="flex items-center gap-2" title="Disabled">
          <div className="w-3 h-3 rounded-full bg-zinc-500 shadow-[0_0_6px_rgba(100,100,100,0.5)]" />
    ...
    if (status === 'testing') {
      return (
        <div className="flex items-center gap-2" title="Testing...">
          <div className="w-3 h-3 rounded-full bg-orange-500 animate-pulse shadow-[0_0_8px_rgba(249,115,22,0.6)]" />
    ...
    if (status === 'problem') {
      return (
        <div className="flex items-center gap-2" title="Error - Completely Down">
    ...
    if (status === 'active' && latency && latency > 2000) {
      return (
        <div className="flex items-center gap-2" title="Slow Response - Moderate Issues">
          <div className="w-3 h-3 rounded-full bg-yellow-500 shadow-[0_0_8px_rgba(234,179,8,0.6)]" />
          <span className="text-xs text-yellow-400">Slow</span>
        </div>
      );
    }
    ...
    if (status === 'active') {
      return (
        <div className="flex items-center gap-2" title="Functioning Normally">
    ...
    return (
      <div className="flex items-center gap-2" title="Inactive">
        <div className="w-3 h-3 rounded-full bg-zinc-600 shadow-[0_0_6px_rgba(100,100,100,0.5)]" />
          ...
          <div className="relative">
            ...
            <div className="absolute -top-1 -right-1">
              <div className={`w-3 h-3 rounded-full ${
                !provider.key ? 'bg-zinc-600' :
                !provider.isEnabled ? 'bg-zinc-500' :
                provider.status === 'testing' ? 'bg-orange-500 animate-pulse' :
                provider.status === 'problem' ? 'bg-red-500' :
                provider.status === 'active' && provider.latency && provider.latency > 2000 ? 'bg-yellow-500' :
                provider.status === 'active' ? 'bg-green-500' : 'bg-zinc-600'
              } shadow-lg`} />
            </div>
          </div>
          <div>
